	...
	if ( ! function_exists( 'gowp_display_admin_page' ) ) {
		function gowp_display_admin_page() {
			...
			<div class="wrap">
				...
				<?php endif; ?>
				<h1><?php echo GOWP_PLUGIN_TITLE; ?></h1>
...
/* UPDATES */
	...
	// Plugin update settings
		...
		add_action( 'admin_init', 'gowp_plugin_settings_init', 100 );
		function gowp_plugin_settings_init() {
			...
			function gowp_settings_updates_description() {
				?>
					<p>If you wish to exclude one or more plugins, or WordPress core updates, from our remote process, you may do so by checking the appropriate boxes below. Excluded updates can still be performed manually upon request.</p>
				<?php
				...
				register_setting( 'gowp_settings', 'gowp_blocked_plugin_updates', 'gowp_set_blocked_plugin_updates' );
				function gowp_block_plugin_updates_render() {
					$gowp_blocked_plugin_updates = (array) gowp_get_blocked_plugin_updates();
					$internal = array( 'gowp-dashboard/gowp-dashboard.php', 'iwp-client/init.php' );
					$plugins = get_plugins();
					$core['core'] = array( 'Name' => 'WordPress Core', 'Version' => get_bloginfo( 'version' ) );
					$updates = array_merge( $core, $plugins );
					echo "<p>";
		...
		function gowp_get_blocked_plugin_updates() {
			$site = home_url();
			$response = wp_remote_get( "https://iwp.gowp.com/gowp.php?action=getBlockedUpdates&site={$site}", array( 'sslverify' => false ) );
			if ( is_array( $response ) ) {
				return unserialize( $response['body'] );
			}
		}

		function gowp_set_blocked_plugin_updates( $options ) {
			...
			if ( is_array( $options ) ) {
				foreach ( $options as $option ) {
					if ( array_key_exists( $option, $plugins ) ) {
						$blocked[$option] = $plugins[$option]['Name'];
					} elseif ( 'core' == $option ) {
						$curl = curl_init();
						curl_setopt( $curl, CURLOPT_URL, "http://wordpress.org/latest" );
						curl_setopt( $curl, CURLOPT_HEADER, TRUE );
						curl_setopt( $curl, CURLOPT_NOBODY, TRUE );
						curl_setopt( $curl, CURLOPT_RETURNTRANSFER, TRUE );
						curl_setopt( $curl, CURLOPT_FOLLOWLOCATION, TRUE );
						$result = curl_exec( $curl );
						if ( preg_match( "/Content-Disposition.*?filename=wordpress-(.*?).tar.gz/", $result, $matches ) ) {
							$blocked[$option] = $matches[1];
						} else {
							$blocked[$option] = get_bloginfo( 'version' );
						}
					}
				}
			}
			$updates = base64_encode( serialize( $blocked ) );
			$site = home_url();
			$response = wp_remote_get( "https://iwp.gowp.com/gowp.php?action=setBlockedUpdates&site={$site}&updates={$updates}", array( 'sslverify' => false ) );
			return $blocked;
		}
	...
	// Prevent us from performing theme or customer-blocked plugin updates

		add_filter( 'http_request_args', 'gowp_block_remote_updates', 10, 2 );
		function gowp_block_remote_updates( $r, $url ) {
			if ( defined( 'IWP_AUTHORISED_CALL' ) ) {
				// Block all remote theme updates
				if ( 0 === strpos( $url, 'https://api.wordpress.org/themes/update-check' ) ) {
					unset( $r['body']['themes'] );
				}
				// Block specific plugin updates
				elseif ( 0 === strpos( $url, 'https://api.wordpress.org/plugins/update-check/' ) ) {
					$plugins = json_decode( $r['body']['plugins'], TRUE );
					$blocked = gowp_get_blocked_plugin_updates();
					if ( ! empty( $blocked ) ) {
	...
	// Prevent us from performing customer-blocked core updates

		add_filter( 'site_transient_update_core', 'gowp_block_remote_core_updates' );
		function gowp_block_remote_core_updates( $value ) {
			if ( defined( 'IWP_AUTHORISED_CALL' ) && isset( $value->updates ) ) {
				$blocked = gowp_get_blocked_plugin_updates();
				if ( in_array( 'core', $blocked ) ) {
					unset( $value->updates );
				}
			}
			return $value;
	...
	// Get update log details from IWP
		...
		function gowp_get_iwp_update_log() {
			$site = home_url();
			$response = wp_remote_get( "https://iwp.gowp.com/gowp.php?action=getUpdates&site={$site}", array( 'sslverify' => false ) );
			if ( is_array( $response ) && ( '200' == $response['response']['code'] ) ) {
				$gowp_iwp_updates_log = array_shift( unserialize( $response['body'] ) );
				update_option( 'gowp_iwp_updates_log', $gowp_iwp_updates_log, FALSE );
			} else {
				$gowp_iwp_updates_log = get_option( 'gowp_iwp_updates_log' );
			}
			return $gowp_iwp_updates_log;
		}
...
/* BACKUPS */
			...
			function gowp_display_backups_tab() {
				...
				foreach ( $hosts as $host ) {
					if ( defined( "{$host['constant']}" ) ) {
						$host_notice = "<p>{$host['name']} provides backup service for this site; these backups can be viewed here: <a href='{$host['panel']}' target='_blank'>{$host['panel']}</a></p>";
				...
				if ( ! isset( $host_notice ) || $host_remote ) {
					...
					if ( isset( $_POST['manual_backup'] ) ) {
						...
						$result = wp_remote_get( $url, array( 'blocking' => false, 'sslverify' => false ) );
						if ( is_array( $result ) ) {
							echo '<p class="gowp-pass">Your manual backup request has been initiated and should appear in the list below shortly.</p>';
						} else {
							echo '<p class="gowp-fail">An error occurred. Please try again or contact support for assistance.</p>';
						}
					}
					if ( $logs = gowp_get_backup_log() ) {
						update_option( 'gowp_backup_log', $logs );
						$message = '<p class="gowp-pass">Successfully connected to backup server. Backups listed below are current.</p>';
					} else {
						$logs = get_option( 'gowp_backup_log' );
						$message = '<p class="gowp-fail">Unable to connect to backup server. Backups listed below may not be current.</p>';
					}
					if ( $logs ) {
						$host = str_replace( array( 'https://', 'http://', 'www.', '.gowpsite.com' ), '', home_url() );
						$logcontents = "";
						foreach ( $logs as $log ) {
						...
						$logcontents = "No recent backup activity found.";
					}
					$logfile = "Backups Log";
					$html = "<h4 class='logfile-name'>{$logfile}</h4>";
					$html .= $message;
	...
	// Build accurate backup log

		function gowp_get_backup_log() {
			$url = "https://iwp.gowp.com/gowp.php?action=getBackupLog&url=" . get_site_url();
			$result = wp_remote_get( $url, array( 'blocking' => true, 'sslverify' => false ) );
			if ( is_array( $result ) && ( '200' == $result['response']['code'] ) ) {
				$logs = unserialize( $result['body'] );
				global $wpdb;
				$iwp_logs = $wpdb->get_col( "SELECT taskResults FROM wp_iwp_backup_status ORDER BY ID DESC;" );
				foreach ( $iwp_logs as $iwp_log ) {
					$details = unserialize( $iwp_log );
					if ( isset( $details['task_results'] ) ) {
						$results = array_shift( $details['task_results'] );
						$iwp[ $results['amazons3'] ] = $results['backup_name'];
					}
				}
				if ( $iwp ) {
					foreach ( $logs as &$log ) {
						if ( array_key_exists( $log['amazons3'], $iwp ) ) {
							$log['backup_name'] = $iwp[ $log['amazons3'] ];
						}
					}
				}
			}
...
/* REPORTING */
	...
	// Add settings
		...
		add_action( 'admin_init', 'gowp_reporting_settings_init', 200 );
		function gowp_reporting_settings_init() {
			if ( ! get_option( 'gowp_enable_whitelabel' ) ) { // Do nothing if this is a white-labelled site
				...
				function gowp_settings_reporting_description() {
					?>
						<p>Each month, we'll email a report to the email address(es) below, summarizing our activities.</p>
					<?php
				}
				add_settings_field(
