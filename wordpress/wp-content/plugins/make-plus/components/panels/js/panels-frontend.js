...
(function($, ttfmpPanels){
	var Panels = {
		...
		init: function() {
			...
			self.cache.$document.ready(function() {
				...
				if (true == self.cache.isCustomizer) {
					self.tabsPlaceholder(self.cache.$tabs);
				}
			});
		},
		 ...
		 * @param $el    jQuery collection of Tabs containers.
		 */
		tabsPlaceholder: function($el) {
			if ('undefined' == typeof ttfmpPanels) {
				return;
			}
			...
			$el.each(function() {
				var $this = $(this),
					...
					$placeholder = $('<div>').addClass('ttfmp-panels-item-content ui-tabs-panel ui-widget-content ui-corner-bottom').html(ttfmpPanels.tabsPlaceholder);
