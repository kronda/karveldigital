...
class TVE_Dash_Product_LicenseManager
{
    ...
    /**
     * @param $item TVE_Dash_Product_Abstract
     * @return bool
     */
    ...
    {
        $api_url = "https://thrivethemes.com/wp-content/plugins/license_check/api/request-v2.php";
    ...
    public function activateProducts(&$response)
    {
        ...
        if (in_array(self::ALL_TAG, $response['products'])) {
            $updated = update_option(self::LICENSE_OPTION_NAME, array(self::ALL_TAG));
        } else {
            $existing = get_option(self::LICENSE_OPTION_NAME, array());
            $updated = update_option(self::LICENSE_OPTION_NAME, array_unique(array_merge($existing, $response['products'])));
        }
