 ...
 *
 * Thrive_Dash_Api_GetResponsePHP is a PHP5 implementation of the Thrive_Dash_Api_GetResponse API
 ...
 * @package Thrive_Dash_Api_GetResponsePHP
 * @version 0.1.1
 */

/**
 * Thrive_Dash_Api_GetResponse Class
 * @package Thrive_Dash_Api_GetResponsePHP
 */
class Thrive_Dash_Api_GetResponse
{
    /**
     * Thrive_Dash_Api_GetResponse API key
    ...
    /**
     * Thrive_Dash_Api_GetResponse API URL
     * @var string
     * @access private
     */
     ...
     * @param string $apiKey Thrive_Dash_Api_GetResponse API key
     * @throws Thrive_Dash_Api_GetResponse_Exception
     */
    public function __construct($apiKey = null)
    {
        if (is_null($apiKey)) {
            throw new Thrive_Dash_Api_GetResponse_Exception('API key must be supplied');
        }
        $this->apiKey = $apiKey;
    }
    ...
    {
        $result = tve_dash_api_remote_post($this->apiURL, array(
            'headers' => array(
        ...
        if ($result instanceof WP_Error) {
            throw new Thrive_Dash_Api_GetResponse_Exception($result->get_error_message());
        }
        $httpCode = $result['response']['code'];
        if (!(($httpCode == '200') || ($httpCode == '204'))) {
            throw new Thrive_Dash_Api_GetResponse_Exception('API call failed. Server returned status code ' . $httpCode);
        }
        $response = json_decode($result['body']);
        if ($response->error) {
            throw new Thrive_Dash_Api_GetResponse_Exception('Api call failed: ' . $response->error->message);
        }
