...
if (!class_exists('Thrive_Leads_State_Switch_Action')) {
    ...
    class Thrive_Leads_State_Switch_Action extends TCB_Event_Action_Abstract
    {
        ...
        public function getJsActionCallback()
        {
            ...
            return "function(t,a,c){var current={$jquery}(this).parents('.tl-style').first();var container=current.parent().find('[data-state='+c.s+']');if(!container.length){return false;}current.hide();container.show().parent().trigger('switchstate',[container,current]);return false;};";
        }
