...
function tve_leads_get_form_layout($current_templates, $post_id, $post_type)
{
    ...
    if (!empty($variation[TVE_LEADS_FIELD_TEMPLATE])) {
        ...
        if (!empty($config['css'])) {
            ...
            tve_leads_enqueue_style($css_handle, TVE_LEADS_URL . 'editor-templates/_form_css/' . $config['css']);
        }
    }
...
function tve_leads_get_editor_template_content(& $variation, $template_key = null)
{
     ...
     */
    $config = tve_leads_get_editor_template_config($template_key);
    if (!empty($config[TVE_LEADS_FIELD_GLOBALS])) {
        $variation[TVE_LEADS_FIELD_GLOBALS] = $config[TVE_LEADS_FIELD_GLOBALS];
    } else {
...
function tve_leads_custom_elements_menu($menu_path)
{
    ...
    $variation_key = $_POST['_key']; // this should always be present
 ...
 */
function tve_leads_tcb_license_override()
{
