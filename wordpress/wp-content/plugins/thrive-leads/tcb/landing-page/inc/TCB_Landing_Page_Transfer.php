...
class TCB_Landing_Page_Transfer
{
    ...
    public function export($page_id, $template_name, $thumbnail_attachment_id = 0)
    {
        ...
        $zip->addFromString('lp.json', base64_encode(serialize($config)));
    ...
    protected function importReadConfig(ZipArchive $zip)
    {
        ...
        $config = unserialize(base64_decode($zip->getFromName('lp.json')));
    ...
    protected function importIconPack($config, ZipArchive $zip)
    {
        ...
        */
        ...
        $this->wpUploadBits(trailingslashit(dirname($font_dir_path)) . 'style.css', $font_css);
        ...
        while ($info = $zip->statIndex($i)) {
            if (strpos($info['name'], 'icon-pack/fonts/') === 0 && !empty($info['size'])) {
                $this->wpUploadBits(trailingslashit($font_dir_path) . basename($info['name']), $zip->getFromIndex($i));
            }
            $i++;
        }
    ...
    protected function importExtraIconPacks(& $config, ZipArchive $zip)
    {
        ...
        foreach ($config['tve_globals']['extra_icons'] as $i => $icon_pack) {
            ...
            /* upload the font css file */
            $this->wpUploadBits(trailingslashit(dirname($font_dir_path)) . 'style.css', $zip->getFromName($family . '/style.css'));
            ...
            while ($info = $zip->statIndex($zip_index)) {
                if (strpos($info['name'], $family . '/fonts/') === 0 && !empty($info['size'])) {
                    $this->wpUploadBits(trailingslashit($font_dir_path) . basename($info['name']), $zip->getFromIndex($zip_index));
                }
                $zip_index++;
            }
    ...
    public function importFontPack($zip, $folder_name, $css_file, $new_folder_name = null)
    {
        ...
        /* upload the font css file */
        $this->wpUploadBits(trailingslashit($font_dir_path) . $css_file, $zip->getFromName($zip_path . '/' . $css_file));
     ...
     */
    protected function findFontByProps($font, $fonts, $field_search = array('font_name', 'font_style', 'font_bold', 'font_color'))
    {
    ...
    {
        return get_option(Thrive_Font_Import_Manager::OPTION_NAME, array());
    }
