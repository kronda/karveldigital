...
class Thrive_Dash_Api_AWeber_Entry extends Thrive_Dash_Api_AWeber_Response
{
    ...
    /**
     * @var array Holds Thrive_Dash_Api_AWeber_Collection objects already instantiated, keyed by
    ...
    public function attrs()
    {
        ...
        if (!empty(Thrive_Dash_Api_AWeber::$_collectionMap[$this->type])) {
            foreach (Thrive_Dash_Api_AWeber::$_collectionMap[$this->type] as $child) {
     ...
     *
     * @access public
     * @param Thrive_Dash_Api_AWeber_Entry(List) List to move Resource (this) too.
     * @param mixed $last_followup_message_number_sent
     * @return mixed Thrive_Dash_Api_AWeber_Entry(Resource) Resource created on List ($list)
     ...
     * @access public
     * @throws Thrive_Dash_Api_AWeber_ResourceNotImplemented
     * @return mixed
     */
    public function __get($value)
    {
        ...
        if (!empty($this->data) && array_key_exists($value, $this->data)) {
            if (is_array($this->data[$value])) {
                $array = new Thrive_Dash_Api_AWeber_EntryDataArray($this->data[$value], $value, $this);
        ...
        throw new Thrive_Dash_Api_AWeber_ResourceNotImplemented($this, $value);
    }
    ...
    public function findSubscribers($search_data)
    {
        ...
        return new Thrive_Dash_Api_AWeber_Collection($data, $url, $this->adapter);
    }
    ...
    protected function _parseNamedOperation($data)
    {
        ...
        foreach ($data as $entryData) {
            $results[] = new Thrive_Dash_Api_AWeber_Entry($entryData, str_replace($this->adapter->app->getBaseUri(), '',
    ...
    protected function _methodFor($entryTypes)
    {
        ...
        throw new Thrive_Dash_Api_AWeber_ResourceNotImplemented($this);
    }

    /**
     * _getCollection
     *
     * Returns the Thrive_Dash_Api_AWeber_Collection object representing the given
    ...
    protected function _getCollection($value)
    {
        if (empty($this->_collections[$value])) {
            $url = "{$this->url}/{$value}";
            $data = $this->adapter->request('GET', $url);
            $this->_collections[$value] = new Thrive_Dash_Api_AWeber_Collection($data, $url, $this->adapter);
        }
    ...
    protected function _isChildCollection($value)
    {
        $this->_type();
        if (!empty(Thrive_Dash_Api_AWeber::$_collectionMap[$this->type]) &&
            in_array($value, Thrive_Dash_Api_AWeber::$_collectionMap[$this->type])
        ) return true;
        return false;
    }
