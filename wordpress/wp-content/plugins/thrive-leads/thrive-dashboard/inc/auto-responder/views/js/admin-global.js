...
(function ($) {
    ...
    TVE_Dash.showLoader = function ($element) {
        if (!$element) {
            return;
        }

        $element.find('.tvd-card').first().addClass("tvd-preloader-overlay");
        $element.find('.tvd-card-content').prepend(_.template($("#tvd-api-form-preloader").html(), {}));
    };

    TVE_Dash.hideLoader = function ($element) {
        if (!$element) {
            return;
        }

        $element.find('.tvd-card-preloader').remove();
        $element.find('.tvd-card').first().removeClass("tvd-preloader-overlay");
    };
    ...
    TVE_Dash.API.views.ConnectionConnected = Backbone.View.extend({
        className: "tvd-col tvd-s12 tvd-m6 tvd-l3",
        events: {
        ...
        render: function () {
            this.$el.html(_.template($("#tvd-api-connected").html(), {item: this.model}));
            return this;
        }
    });
    ...
    TVE_Dash.API.views.ConnectionTest = Backbone.View.extend({
        ...
        render: function () {
            this.$el.html(_.template($("#tvd-api-connection-hover").html(), {
        ...
        test: function () {
            ...
            ajax.done(function (response) {
                ...
                if (response.success) {
                    ...
                    model.color = 'tvd-green';
                    model.icon = 'tvd-icon-check';
                    self.$el.html(_.template($("#tvd-api-connection-hover").html(), model));
                } else {
    ...
    TVE_Dash.API.views.ConnectionError = Backbone.View.extend({
        ...
        render: function () {
            this.$el.html(_.template($("#tvd-api-connection-error").html(), {item: this.model}));
            return this;
        }
    });
    ...
    TVE_Dash.API.views.ConnectionDelete = Backbone.View.extend({
        ...
        render: function () {
            this.$el.html(_.template($("#tvd-api-confirm-delete").html(), {item: this.model}));
            return this;
        },
        yes: function () {
            var self = this,
            ...
            this.$el.find('.tvd-card').replaceWith(_.template($("#tvd-api-connection-hover").html(), {
    ...
    TVE_Dash.API.views.ConnectionDone = Backbone.View.extend({
        ...
        render: function () {
            this.$el.html(_.template($("#tvd-api-state-success").html(), {item: this.model}));
            return this;
        }
    });
    ...
    TVE_Dash.API.views.NewState = Backbone.View.extend({
        ...
        render: function () {
            this.$el.html(_.template($("#tvd-api-state-new").html(), {item: this.model}));
            return this;
        }
    });
    ...
    TVE_Dash.API.views.SelectState = Backbone.View.extend({
        ...
        render: function () {
            this.$el.html(_.template($("#tvd-api-state-select").html(), {
                item: this.model
            }));
            this.renderAPIsList();
        ...
        renderAPIsList: function () {
            ...
            TVE_Dash.API.APITypes.each(function (type) {
                ...
                this.collection.getAPIs(false).getByType(type.get('type')).each(function (api) {
    ...
    TVE_Dash.API.views.FormState = Backbone.View.extend({
        ...
        render: function (editMode) {
            this.$el.html(_.template($("#tvd-api-state-form").html(), {item: this.model}));
            this.$el.find('.tvd-card-content').prepend(_.template($("#tvd-api-form-" + this.model.get("key")).html(), {item: this.model}));
            if (editMode !== true && this.model.get('key' !== 'awsses')) {
    ...
    TVE_Dash.API.views.ErrorState = Backbone.View.extend({
        ...
        render: function () {
            this.$el.html(_.template($("#tvd-api-state-error").html(), {item: this.model}));
            return this;
        }
    });
