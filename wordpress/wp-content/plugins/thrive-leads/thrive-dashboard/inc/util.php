 ...
 *
 * the count for social shares will be stored in a post_meta field: thrive_ss_count
 *
 ...
 * @param bool $force_fetch if true, it will bypass the cache and make the API requests
 *
 ...
 * ..
 */
function thrive_util_get_share_count($post_id, $post_permalink = null, $networks = null, $force_fetch = false)
{
    /* half an hour */
    $cache_lifetime = apply_filters('thrive_cache_shares_lifetime', 1800);
    ...
    if ($force_fetch) {
        ...
        foreach ($networks as $network) {
            $shares = call_user_func('thrive_util_fetch_count_' . $network, $post_permalink);
 ...
 * @return int
 */
function thrive_util_fetch_count_fb_share($url)
{
    $data = _thrive_util_helper_get_json('http://graph.facebook.com/?id=' . rawurlencode($url));
 ...
 * @return int
 */
function thrive_util_fetch_count_t_share($url)
{
    return 0;
}
 ...
 * @return int
 */
function thrive_util_fetch_count_pin_share($url)
{
 ...
 * @return int
 */
function thrive_util_fetch_count_in_share($url)
{
    $data = _thrive_util_helper_get_json('http://www.linkedin.com/countserv/count/share?format=json&url=' . rawurlencode($url));
 ...
 * @return int
 */
function thrive_util_fetch_count_g_share($url)
{
 ...
 * @return int
 */
function thrive_util_fetch_count_xing_share($url)
{
 ...
 * @return array
 */
function _thrive_util_helper_get_json($url, $fn = 'wp_remote_get')
{
