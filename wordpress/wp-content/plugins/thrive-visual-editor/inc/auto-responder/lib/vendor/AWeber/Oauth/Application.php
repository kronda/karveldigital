 ...
 * Thrive_Api_AWeber_Oauth_ServiceProvider should have a complementary OAuthApplication
 *
 ...
 * @version $id$
 */
class Thrive_Api_AWeber_Oauth_Application implements Thrive_Api_AWeber_Oauth_Adapter
{
    public $debug = false;
    ...
    /**
     * @var Thrive_Api_AWeber_Oauth_User User currently interacting with the service provider
     */
    public $user = false;
     ...
     * __construct
     *
     * Create a new OAuthApplication, based on an Thrive_Api_AWeber_Oauth_ServiceProvider
     * @access public
     *
    ...
    {
        if ($parentApp) {
            if (!is_a($parentApp, 'Thrive_Api_AWeber_Oauth_ServiceProvider')) {
                throw new Exception('Parent App must be a valid Thrive_Api_AWeber_Oauth_ServiceProvider!');
            }
            $this->app = $parentApp;
        }
        $this->user = new Thrive_Api_AWeber_Oauth_User();
    }
    ...
    public function request($method, $uri, $data = array(), $options = array())
    {
        ...
        if (empty($options['allow_empty']) && !isset($data)) {
            throw new Thrive_Api_AWeber_ResponseError($uri);
        }
        return $data;
    }
    ...
    public function getAccessToken()
    {
        ...
        if (empty($data['oauth_token'])) {
            throw new Thrive_Api_AWeber_OAuthDataMissing('oauth_token');
        }
    ...
    public function parseAsError($response)
    {
        if (!empty($response['error'])) {
            throw new Thrive_Api_AWeber_OAuthException($response['error']['type'],
    ...
    protected function requiredFromResponse($data, $requiredFields)
    {
        foreach ($requiredFields as $field) {
            if (empty($data[$field])) {
                throw new Thrive_Api_AWeber_OAuthDataMissing($field);
            }
        }
    }
    ...
    public function makeRequest($method, $url, $data = array())
    {
        ...
        switch (strtoupper($method)) {
            ...
            case 'PATCH':
                ...
                break;
            default:
                throw new Thrive_Api_AWeber_APIException('Method not implemented: ' . $method, $url);
        }
        ...
        $resp = new Thrive_Api_AWeber_Http_Response($response);
        ...
        if (!$resp) {
            ...
            throw new Thrive_Api_AWeber_APIException($error, $url);
        }
        if ($resp->headers['Status-Code'] >= 400) {
            $data = json_decode($resp->body, true);
            if ($data) {
                throw new Thrive_Api_AWeber_APIException($data['error'], $url);
            } else {
