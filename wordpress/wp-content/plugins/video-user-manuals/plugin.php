...
class Vum{
	...
	const iframe_frontend_embed_url = 'http://wordpress.videousermanuals.com/iframe-embed-frontend.php?jsoncallback=?';
	const iframe_frontend_embed_user = 'http://wordpress.videousermanuals.com/player-embed-iframe-user.php?'; // <iframe> URL to embed user video
	...
	function admin_scripts() {
		...
		//Add Onpage script
		$this->onpage_admin_init();
		
		wp_enqueue_script( 'vum-admin-script' );
	...
	function onpage_admin_init( ) {
		...
		$json_frontend_url = apply_filters( 'vum_domain', self::vum_domain . 'iframe-onpage.php?jsoncallback=?' );
		...
		$translation_array = array(
			'vum_url' => $json_frontend_url,
			'vum_params' => (array) $url
		);
		wp_localize_script( 'vum-admin-script', 'vum_object', $translation_array );
    ...
    function defineForm()
    {
		...
		if($embed_status->status == 4){
			...
			$form->html( '<div id="video-player-settings" style="clear:both">' );$form->html( '<h2>Video Player</h2>' );
			$form->html( '<p>
			...
			$form->html( '<p class="submit"><input type="submit" name="submit-button" class="button-primary" value="Save Changes"></p>' );
			$form->html( '</div>' );
	...
	function display_vid( $video_id, $video, $is_pop_up = true ) {
		...
		if ( $video->doembed == '1' ) {
			...
			$onclick = self::vum_domain . 'video-player.php?video_url=' . $video->vid . '&amp;video_thumb=' . $video->image . '&amp;width=' . ( $width + 25 ) . '&amp;height=' . $height . '\',\'welcome\',\'width=' . ( $width + 50 ) . ', height=' . ( $height + 50 ) . ',menubar=0,status=0,location=0,toolbar=0,scrollbars=0';
		}
	...
	function display_vid_embedded( $video_id, $video ) {
		...
		$embed_url = apply_filters( 'vum_embed_iframe', self::iframe_frontend_embed_user );
	...
	function video_shortcode($atts) {
	   ...
	   if(!empty($ids))
		...
		if ( get_option( 'wpm_o_form_embed_video' ) ) {
			$json_frontend_url = apply_filters( 'iframe_frontend_url', self::iframe_frontend_embed_url );
		} else {
			...
			jQuery(document).ready(function(){
				...
				jQuery("#contextual-help-link-wrap").on("click", function(){
			        if( jQuery("#tab-panel-vum-videos #vum-videos").html() == "" ) {
			            ...
			            jQuery("<div />", {"id":"vum-player-wrapper"}).html('<iframe src="" frameborder="0" id="targetvum_player_frame" name="targetvum_player_frame" style="overflow:hidden;height:100%;width:100%" height="100%" width="100%"></iframe>').appendTo("#vum-modal");
			          
						jQuery.getJSON(vum_object.vum_url, vum_object.vum_params, function(data, textStatus) {
