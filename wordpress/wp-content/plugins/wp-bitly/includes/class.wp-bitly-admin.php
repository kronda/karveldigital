...
class WP_Bitly_Admin {
	...
	/**
	 * Display a notice on the plugins page after activation (and up until they add their oauth_token).
	 *
	 * @since   2.0
	 */
	public function display_notice() {
		...
		$text = sprintf( __( 'WP Bitly is almost ready! Please visit the %s to add your oauth token.', 'wp-bitly' ), $link );
		$message = '<div id="message" class="updated"><p>' . $text . '</p></div>';

		$message = apply_filters( 'wpbitly_setup_notice', $message, $text );
	...
	public function register_settings() {
		...
		function _f_settings_section() {
			echo '<p>' . __( 'You will need a Bitly account to use this plugin. Click the link below for your OAuth Token, and if necessary create a new account.', 'wp-bitly' ) . '</p>';
		}
		...
		function _f_settings_field_oauth() {
			...
			$css = $authorized ? '' : ' style="border-color: #c00; background-color: #ffecec;" ';
			$css = apply_filters( 'wpbitly_oauth_css', $css, $authorized );

			$output = '<input type="text" size="80" name="wpbitly-options[oauth_token]" value="' . esc_attr( $wpbitly->get_option( 'oauth_token' ) ) . '"' . $css . '>'
		...
		function _f_settings_field_post_types() {
			...
			$post_types = apply_filters( 'wpbitly_allowed_post_types', $post_types );
			$current_post_types = $wpbitly->get_option( 'post_types' );

			$output = '<fieldset><legend class="screen-reader-text"><span>' . __( 'Post Types', 'wp-bitly' ) . '</span></legend>';
			foreach ( $post_types as $label ) {
				$output .= '<label for "' . $label . '>';
				$output .= '<input type="checkbox" name="wpbitly-options[post_types][]" value="' . $label . '" ' . checked( in_array( $label, $current_post_types ), true, false ) . '>' . $label . '</label><br>';
			}
		...
		function _f_settings_field_debug() {
			...
			$output = '<fieldset><legend class="screen-reader-text"><span>' . __( 'Debug WP Bitly', 'wp-bitly' ) . '</span></legend>';
			$output .= '<label title="debug"><input type="checkbox" id="debug" name="wpbitly-options[debug]" value="1" ' . checked( $wpbitly->get_option( 'debug' ), 1, 0 ) . '><span> ' . __( "Let's debug!", 'wpbitly' ) . '</span></label><br>';
			$output .= '<p class="description">' . __( "If you're having issues generating shortlinks, turn this on and create a thread in the ", 'wpbitly' );
			$output .= '<a href="http://wordpress.org/support/plugin/wp-bitly" title="' . __( 'WP Bitly support forums on WordPress.org', 'wpbitly' ) . '">' . __( 'support forums', 'wpbitly' ) . '</a>.</p>';
			$output .= '</fieldset>';
	...
	public function add_metaboxes_yo( $post ) {
		...
		add_meta_box( 'wpbitly-meta', __( 'WP Bitly', 'wp-bitly' ), array( $this, 'display_metabox' ), $post->post_type, 'side', 'default', array( $shortlink ) );
	}
	...
	public function display_metabox( $post, $args ) {
		...
		if ( isset( $clicks ) && isset( $refer ) ) {
			...
			echo '<p class="error">' . __( 'There was a problem retrieving information about your link. There may not be statistics yet.', 'wp-bitly' ) . '</p>';
		}
