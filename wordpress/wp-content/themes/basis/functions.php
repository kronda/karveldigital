...
function basis_archives_title( $echo = true ) {
	...
	if ( is_category() ) {
		...
		$result = sprintf( __( 'From <strong>%s</strong>', 'basis' ), get_the_time( 'F jS, Y' ) );
	} elseif ( is_month() ) {
		$result = sprintf( __( 'From <strong>%s</strong>', 'basis' ), get_the_time( 'F, Y' ) );
	} elseif ( is_year() ) {
		$result = sprintf( __( 'From <strong>%s</strong>', 'basis' ), get_the_time( 'Y' ) );
	} elseif ( is_author() ) {
...
function basis_fitvids_script_setup( $name ) {
	wp_enqueue_script(
		$name,
		get_template_directory_uri() . '/includes/javascripts/fitvids/jquery.fitvids.js',
		array( 'jquery' ),
		'1.0.3',
		true
	);
	...
	<div id="comment-<?php comment_ID(); ?>">
		<article class="respond-body">
			...
			printf(
				...
				get_comment_time( 'c' ),
				get_comment_date( __( 'F jS, Y', 'basis' ) )
			);
			?>
			</footer>
		</article>
...
function basis_is_sidebar_view() {
	...
	// Return true if it's a page with a featured image using the default page template
	if ( is_page() && '' !== get_post_thumbnail_id() && '' === get_page_template_slug() ) {
		return true;
	}
