	...
	function css_editor_activate() {
		...
		if( typeof editor === 'undefined' ) {
			...
			$('.CodeMirror').bind('keyup paste', function(e) {
				if (e.type == 'paste') {
					setTimeout(css_builder_custom_css_change2, 30);
				} else {
					css_builder_custom_css_change2();
				}
			});
		...
		var css_builder_element_selectors_enable_handler = function() {
			...
			if( clickCounter == 0 ) {
				...
				$('.element-labels').click(function() {
					...
					var label_html = $(this).closest('div[id]').clone().wrap('<div></div>').parent().html().replace(/\t/g,'').replace(/ style="position: relative;"/g,'').replace(/ style="position: relative; "/g,'').replace(/ class=""/g,'');
					$('#css-builder-html').val(label_html);
				});
				clickCounter = 1;
			} else {
